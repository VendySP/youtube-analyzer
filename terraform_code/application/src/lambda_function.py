...
def lambda_handler(event, context):
    try:
        target_count = int(os.environ.get('TARGET_COMMENT_COUNT', 10))
    except (ValueError, TypeError):
        target_count = 10 
        
    api_request_buffer = target_count + 5
    ...
    try:
        ...
        request = youtube.commentThreads().list(
            part="snippet", videoId=video_id, maxResults=api_request_buffer, order="relevance", textFormat="plainText"
        )
        response = request.execute()
        ...
        if comments_list:
            ...
            prompt = f"""You are an expert audience analyst. Analyze the following {len(comments_list)} comments for the YouTube video titled "{meta['video_title']}" by creator "{meta['creator_name']}". 
            Provide a precise, 2-sentences summary explaining the audience's primary reactions and what they think about the content. Do not mention individual usernames. Keep it objective.
